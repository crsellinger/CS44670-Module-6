...
DATASET_NAME: Final[str] = "sergionefedov/global-semiconductor-industry-2010-2026"

# STEP 1. Pick the target variable we want to predict.
TARGET_COL: Final[str] = "fab_type"
...
MODEL_PATH: Final[Path] = Path("artifacts") / "csellinger_model.joblib"
...
def load_data() -> pd.DataFrame:
    ...
    # drop rows with zero values in the feature columns
    df_model: pd.DataFrame = df_model[(df_model[FEATURE_COLS] != 0).all(axis=1)]
    # take the top 4 fat types by count
    top_fab_types: list[str] = (
        df_model[TARGET_COL].value_counts().nlargest(4).index.tolist()
    )
    df_model: pd.DataFrame = df_model[df_model[TARGET_COL].isin(top_fab_types)].copy()
